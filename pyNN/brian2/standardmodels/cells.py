...
adexp_iaf = brian2.Equations('''
                dv/dt = ((v_rest-v) + delta_T*exp((v - v_thresh)/delta_T))/tau_m + (i_syn + i_offset + i_inj - w)/c_m : volt
...
adapt_iaf = brian2.Equations('''
                dv/dt = (v_rest-v)/tau_m + (-g_r*(v-E_r) - g_s*(v-E_s) + i_syn + i_offset + i_inj)/c_m  : volt
...
class Izhikevich(cells.Izhikevich):
    ...
    eqs = brian2.Equations('''
        dv/dt = (0.04/ms/mV)*v**2 + (5/ms)*v + 140*mV/ms - u + (i_offset + i_inj)/pF : volt
