# coding: utf-8
"""
This is an implemention of benchmarks 1 and 2 from the Brette et al. (2007)
review paper (Journal of Computational Neuroscience, in press).
...
if benchmark == "COBA":
    celltype = IF_cond_alpha          # Since for the moment no IF_cond_exp model
    w_exc = Gexc                      #  is available for nest
    w_inh = Ginh
elif benchmark == "CUBA":
...
connections = {
    'e2e' : Projection(exc_cells, exc_cells,'fixedProbability', pconn, target='excitatory',rng=rng),
...
print "%d Number of connections: %d e→e  %d e→i  %d i→e  %d i→i" % (node_id, len(connections['e2e']),
                                                                    len(connections['e2i']),
                                                                    len(connections['i2e']),
                                                                    len(connections['i2i']))
