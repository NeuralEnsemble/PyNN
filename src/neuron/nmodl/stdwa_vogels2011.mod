...
NEURON {
	...
	RANGE deltaw, wmax, tau, eta, rho, on
...
PARAMETER {
	...
	rho 	= 0.2		 : strength of non-Hebbian synaptic depression relative to Hebbian potentiation.
...
NET_RECEIVE (w) {
	if (w >= 0) {				: this is a pre-synaptic spike
		...
		tlast_pre = t
		deltaw = deltaw + eta*(exp(interval/tau) - rho)
