	...
	Prop *_prop;
{
 ...
 pnt_receive_size[_mechtype] = 5;
 	...
 	ivoc_help("help ?1 tmgsyn /home/andrew/dev/pyNN_trunk/src/hoc/i686/tmgsyn.mod\n");
...
{    _p = _pnt->_prop->param; _ppvar = _pnt->_prop->dparam;
 ...
 _tsav = t; {
   ...
   if ( tau_facil > 0.0 ) {
     state_discontinuity ( -1, & _args[3] , _args[3] + U * ( 1.0 - _args[3] ) ) ;
     }
   state_discontinuity ( _cvode_ieq + 0, & g , g + _args[0] * x * _args[3] ) ;
   state_discontinuity ( -1, & _args[1] , _args[1] + x * _args[3] ) ;
   _args[4] = t ;
   } }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  state();
 if(error){fprintf(stderr,"at line 131 in file tmgsyn.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
