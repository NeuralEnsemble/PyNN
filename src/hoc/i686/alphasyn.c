...
#define g _p[3]
#define q _p[4]
#define onset_times (_p + 5)
#define weight_list (_p + 105)
#define v _p[205]
#define _g _p[206]
#define _tsav _p[207]
...
#define alpha alpha_AlphaSyn
#define update_queue update_queue_AlphaSyn
 extern double alpha();
 extern double update_queue();
	...
	Prop *_prop;
{
  ...
  if (nrn_point_prop_) {
	...
	_ppvar = nrn_point_prop_->dparam;
 	...
 	_p = nrn_prop_data_alloc(_mechtype, 208, _prop);
 	/*initialize range parameters*/
 	tau = 5;
 	e = 0;
  }
 	_prop->param = _p;
 	_prop->param_size = 208;
  if (!nrn_point_prop_) {
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 AlphaSyn /home/andrew/dev/pyNN_trunk/src/hoc/i686/alphasyn.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Alpha-function synaptic conductance, with NET_RECEIVE";
...
double update_queue ( _p, _ppvar, _thread, _nt, _ln ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _ln ;
 {
...
static double _hoc_update_queue(_vptr) void* _vptr; {
 double _r;
...
double alpha ( _p, _ppvar, _thread, _nt, _lx ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lx ;
 {
   double _lalpha;
 if ( _lx < 0.0 ) {
...
static double _hoc_alpha(_vptr) void* _vptr; {
 double _r;
   double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
   _p = ((Point_process*)_vptr)->_prop->param;
  _ppvar = ((Point_process*)_vptr)->_prop->dparam;
 ...
 g = 0.0 ;
   ...
   {int  _lk ;for ( _lk = 0 ; _lk <= ((int) q ) - 1 ; _lk ++ ) {
     ...
     if ( _lx > 20.0 ) {
       ...
       g = g + weight_list [ _lk ] * alpha ( _threadargscomma_ _lx ) ;
       }
     } }
   i = g * ( v - e ) ;
