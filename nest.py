...
$Id: nest.py 14 2007-01-30 13:09:03Z apdavison $
"""
...
class Population(common.Population):
    ...
    def __init__(self,dims,cellclass,cellparams=None,label=None):
        ...
        self.cell = [ pynest.getGID(addr) for addr in self.cell ]
    ...
    def __getitem__(self,addr):
        ...
        if isinstance(addr, int):
            return self.cell.reshape((self.cell.size,))[addr]
        elif isinstance(addr,tuple) or isinstance(addr,list):
            if len(addr) == self.ndim:
                return self.cell[addr]
            else:
                raise common.InvalidDimensionsError, "Population has %d dimensions" % self.ndim
...
class Projection(common.Projection):
    ...
    def setWeights(self,w):
        ...
        if type(w) == types.FloatType or type(w) == types.IntType:
            w = w*1000 # weights should be in nA or uS, but iaf_neuron uses pA and iaf_cond_neuron uses nS.
                       # Using convention in this way is not ideal. We should be able to look up the units used by each model somewhere.
